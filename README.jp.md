...
このプロジェクトは個人管理のコミュニティプロジェクトですが、Microsoft Open Source で示されている貢献の考え方を参考に、建設的で安全な参加を歓迎します。

- バグ報告、改善提案、ドキュメント修正、機能追加の Pull Request を歓迎します。
- 変更前に Issue または Discussion で背景と方針を共有すると、レビューが進めやすくなります。
- Pull Request には目的、主な変更点、確認した動作、関連 Issue を記載してください。
- 既存の構成、命名規則、日英対応、セキュリティ上の注意に沿った最小差分を心がけてください。
- AI 生成コンテンツや支援ツールを使用した場合も、投稿者自身が内容を確認し、正確性と適切性に責任を持ってください。

### 行動規範

本プロジェクトでは [Microsoft Open Source Code of Conduct](https://opensource.microsoft.com/codeofconduct/) を参照し、すべての参加者に敬意ある協力的なコミュニケーションを求めます。ハラスメント、不適切な言動、個人情報の無断公開などは許容されません。
