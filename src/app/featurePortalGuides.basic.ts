...
export const BASIC_GUIDES: Record<string, FeatureGuide> = {
  ...
  'index-builder': {
    ...
    steps: [
      { icon: 'box-arrow-in-right', titleJa: '開く', titleEn: 'Open', descJa: 'ツールメニューまたはこのカードから Index Builder を開きます。接続中の Search サービスのインデックスを管理します。', descEn: 'Open Index Builder from the Tools menu or this card. It manages indexes in the connected search service.' },
      { icon: 'list-ul', titleJa: 'インデックスを選ぶ', titleEn: 'Select an index', descJa: '左ペインで既存インデックスを選択します。右ペインには統計、操作ボタン、編集タブが表示されます。', descEn: 'Select an existing index in the left pane. The right pane shows stats, actions, and editing tabs.', targetSelector: '[data-guide-target="index-builder-list"]' },
      { icon: 'table', titleJa: 'Field Matrix で field を編集', titleEn: 'Edit fields in Field Matrix', descJa: 'Schema Workbench の Field Matrix で属性を切り替え、Lexical settings や Vector Profile / Dim を field ごとに編集します。', descEn: 'Use Field Matrix in Schema Workbench to toggle attributes and edit Lexical settings or Vector Profile / Dim per field.', targetSelector: '[data-guide-target="index-builder-schema"]' },
      { icon: 'ui-checks-grid', titleJa: '構成エディターを使う', titleEn: 'Use Configuration Editors', descJa: 'Semantic、Scoring Profiles、Suggesters、Analyzers、Normalizers、Vector profiles などの index レベル設定をフォームで編集できます。', descEn: 'Edit index-level settings such as Semantic, Scoring Profiles, Suggesters, Analyzers, Normalizers, and Vector profiles through forms.', targetSelector: '[data-guide-target="index-builder-config"]' },
      { icon: 'code-slash', titleJa: 'JSON を確認する', titleEn: 'Review JSON', descJa: 'JSON エディターでは最終的に送信するインデックス定義を確認・直接編集できます。インポート / エクスポートもここから使います。', descEn: 'Use the JSON editor to review or directly edit the final index definition. Import and export are available from the actions bar.', targetSelector: '[data-guide-target="index-builder-editor"]' },
      { icon: 'cloud-upload', titleJa: 'Diff を見て保存', titleEn: 'Save with diff review', descJa: 'Save を押すと、既存リソースとの差分を確認してから作成または更新します。', descEn: 'Click Save to review the diff before creating or updating the index.', targetSelector: '[data-guide-target="index-builder-actions"]' },
    ],
    tipsJa: ['Lexical settings の Analyzer / Normalizer / Synonym maps は候補付き入力です', 'Synonym maps から Synonym Map Builder、Vector Profile / Dim から Vector profiles 構成へ移動できます', 'Aliases タブでアプリケーションが参照するインデックス名を切り替えられます'],
    tipsEn: ['Analyzer / Normalizer / Synonym maps in Lexical settings provide selectable suggestions', 'Open Synonym Map Builder from Synonym maps, and Vector profiles from Vector Profile / Dim', 'Use the Aliases tab to switch the index name referenced by applications'],
  ...
  'indexing-pipeline-builder': {
    ...
    steps: [
      { icon: 'box-arrow-in-right', titleJa: '開く', titleEn: 'Open', descJa: 'ツールメニューまたはこのカードから Indexing Pipeline Builder を開きます。データソース、インデックス、インデクサーを 1 つの取り込みパイプラインとして扱います。', descEn: 'Open Indexing Pipeline Builder from the Tools menu or this card. It treats the data source, index, and indexer as one indexing pipeline.' },
      { icon: 'arrow-repeat', titleJa: '既存インデクサーを読み込む', titleEn: 'Load an indexer', descJa: '既存インデクサーを選択して読み込むと、参照先のデータソースとターゲットインデックスもまとめて確認できます。', descEn: 'Select and load an existing indexer to inspect its data source and target index together.', targetSelector: '[data-guide-target="ipb-resources"]' },
      { icon: 'hdd-network', titleJa: 'データソースを設定', titleEn: 'Configure the data source', descJa: 'Source ノードで種類、接続、認証、コンテナーなどのスコープを設定します。接続文字列または Managed Identity を選択できます。', descEn: 'Use the Source node to configure type, connection, authentication, and scope such as a container. Choose connection string or Managed Identity.', targetSelector: '[data-guide-target="ipb-node-source"]' },
      { icon: 'shuffle', titleJa: 'マッピングを整える', titleEn: 'Align mappings', descJa: 'Indexer ノードで dataSourceName、targetIndexName、skillsetName、fieldMappings、outputFieldMappings を設定します。', descEn: 'Use the Indexer node to configure dataSourceName, targetIndexName, skillsetName, fieldMappings, and outputFieldMappings.', targetSelector: '[data-guide-target="ipb-node-indexer"]' },
      { icon: 'table', titleJa: 'ターゲット Index を確認', titleEn: 'Check the target index', descJa: 'Target ノードでは確認のみ行います。Indexer 側で設定した targetIndexName と draft Index の name が一致しているか、Key Field と Field 一覧を確認します。', descEn: 'Use the Target node for confirmation only. Check that the indexer targetIndexName matches the draft index name, then review the key field and field list.', targetSelector: '[data-guide-target="ipb-node-index"]' },
      { icon: 'play-circle', titleJa: '実行方法を選ぶ', titleEn: 'Choose how to run', descJa: 'Indexer 実行は既存インデクサーの実行のみです。Publish & Run pipeline は差分確認後にリソースを公開してから実行します。', descEn: 'Indexer Run only runs the existing indexer. Publish & Run pipeline reviews diffs, publishes resources, then runs.', targetSelector: '[data-guide-target="ipb-publish-run"]' },
      { icon: 'shield-check', titleJa: '結果を検証', titleEn: 'Verify results', descJa: 'Overview で Run tracker、ステータス、投入件数、サンプルドキュメントを確認します。', descEn: 'Use Overview to inspect the run tracker, status, document count, and sample documents.', targetSelector: '[data-guide-target="ipb-node-verify"]' },
    ],
    tipsJa: ['既存リソースの更新は Diff 確認後にのみ実行します', 'Azure portal の Storage Account URL を貼り付けても接続先の正規化が行われます', '詳細な index スキーマ編集は Index Builder、skillset 編集は Skill Pipeline Builder と組み合わせてください'],
    tipsEn: ['Existing resources are updated only after diff review', 'Storage Account URLs copied from the Azure portal are normalized for the connection', 'Use Index Builder for detailed index schema edits and Skill Pipeline Builder for skillset editing'],
  ...
  'index-cluster-visualizer': {
    ...
    steps: [
      ...
      { icon: 'play-circle', titleJa: 'スキャンして散布図を生成', titleEn: 'Scan & generate scatter plot', descJa: '実行ボタンで文書ベクトルを読み込み、似た文書ごとのまとまりを散布図にします。', descEn: 'Click Run to load document vectors and plot groups of similar documents.', targetSelector: '[data-guide-target="iv-run"]' },
      ...
      { icon: 'chat-left-text', titleJa: 'LLM で要約して登録', titleEn: 'Summarize and register with LLM', descJa: '各クラスタの代表文書から名前・概要・キーワードを作成し、メタインデックスとして Azure AI Search に登録します。', descEn: 'Create a name, summary, and keywords from representative documents in each cluster, then register them as a meta-index in Azure AI Search.', targetSelector: '[data-guide-target="iv-meta"]' },
      { icon: 'download', titleJa: '保存形式を使い分ける', titleEn: 'Use the right save format', descJa: '.ragvis.json は可視化構造、.ragmeta.json は LLM 要約・Trace の再利用に使います。', descEn: '.ragvis.json is for visualization structure; .ragmeta.json is for reusing LLM summaries and traces.' },
    ],
    tipsJa: ['まず 500〜1000 件で実行して傾向を掴みましょう', 'チャンクインデックスでは Adaptive Sampling を有効に', 'LLM 再実行を避けたい場合は Meta JSON を保存しましょう'],
    tipsEn: ['Start with 500–1000 documents to get a feel', 'Enable Adaptive Sampling for chunk indexes', 'Save Meta JSON when you want to avoid repeated LLM runs'],
    docsUrl: 'https://learn.microsoft.com/azure/search/vector-search-overview',
  },
