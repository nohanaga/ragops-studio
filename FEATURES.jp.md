...
- **表比較**: `query` / `semantic` / `vector` / `hybrid` / `semantic_hybrid` / `agentic_retrieve` の順位付き結果を列単位で比較
  - 同一文書の順位変化、検索スコア、リランカースコア、応答時間、RAW 結果を表示
  - スコア尺度は検索モードごとに異なるため、異種モード間では順位と同一文書の移動を中心に比較
- **ラン削除**: 個別ランの削除
