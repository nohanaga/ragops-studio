...
Azure AI Search のスキルセットをビジュアルなフローエディターでオーサリングできるビルダーです。各スキルをノード、入出力をエッジとして左から右への DAG（有向非巡回グラフ）で可視化します。

- **ビジュアルなフローエディター**: ReactFlow + dagre による自動レイアウトの DAG 表示
